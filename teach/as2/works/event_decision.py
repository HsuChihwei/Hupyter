...
class Event(object):
    ...
    def __str__(self):
        """pass"""
        return "Event({} @ {}, {}, {})".format(self._name, self._time, self._outdoors, self._cover_available)
...
class UserInteraction(object):
        ...
        for index, i in enumerate(question_list):
            ...
            if index == 0:
                ...
                try:
                    ...
                    error = True
            if error:
                print('Error input, please input valid value!')
                return self._get_user_answer(result=result)
            result.append(value)
        return result
    ...
    def output_advisability(self, impact):
        ...
        string = "Based on the {} model, the advisability of holding {} is {}.\n"
        print(string.format(type(self._prediction_model).__name__, self._event.get_name(), impact))
#         print("based on the", type(self._prediction_model).__name__, "model","the advisability of holding" ,self._event.get_name(), "is" ,impact,)
    ...
    def another_check(self):
        ...
        """
        user_choice = input("would you like to check again?[Y/N] ").strip()
